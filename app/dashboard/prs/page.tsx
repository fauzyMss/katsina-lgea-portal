    ...
    <div className="min-h-screen bg-gray-50">
      <nav className="bg-green-800 text-white px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <span className="text-sm">HOD PRS</span>
      ...
      <div className="flex">
        <aside className="w-64 min-h-screen bg-white shadow p-4">
          {['Dashboard','Staff Records','Leave Applications','Promotions','Transfers','Complaints','Reports','Announcements'].map((item) => (
        ...
        <main className="flex-1 p-6">
          <h1 className="text-2xl font-bold text-gray-800 mb-2">PRS Dashboard</h1>
          <p className="text-gray-500 text-sm mb-6">Personnel Records & Statistics</p>
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            {[
              ...
              {label:'Teachers',value:'0',bg:'bg-green-50',border:'border-green-200'},
              {label:'LEA Admin Staff',value:'0',bg:'bg-teal-50',border:'border-teal-200'},
            ].map((s) => (
              <div key={s.label} className={`${s.bg} border ${s.border} rounded-xl p-4`}>
                <p className="text-3xl font-bold text-gray-800">{s.value}</p>
                <p className="text-sm text-gray-500 mt-1">{s.label}</p>
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5 mb-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                {label:'View All Staff',color:'bg-blue-600'},
                {label:'Generate Report',color:'bg-green-600'},
              ].map((a) => (
