    ...
    <div className="min-h-screen bg-gray-50">
      <nav className="bg-green-800 text-white px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <span className="text-sm">HOD Personnel</span>
      ...
      <div className="flex">
        <aside className="w-64 min-h-screen bg-white shadow p-4">
          {['Dashboard','Staff Records','Leave Applications','Promotions','Transfers','Complaints','Invite Codes','Reports','Announcements'].map((item) => (
        ...
        <main className="flex-1 p-6">
          <h1 className="text-2xl font-bold text-gray-800 mb-2">HOD Personnel Dashboard</h1>
          <p className="text-gray-500 text-sm mb-6">Personnel Department — Full Access</p>
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            {[
              ...
              {label:'Active Invite Codes',value:'0',bg:'bg-teal-50',border:'border-teal-200'},
              {label:'New Staff This Month',value:'0',bg:'bg-green-50',border:'border-green-200'},
            ].map((s) => (
              <div key={s.label} className={`${s.bg} border ${s.border} rounded-xl p-4`}>
                <p className="text-3xl font-bold text-gray-800">{s.value}</p>
                <p className="text-sm text-gray-500 mt-1">{s.label}</p>
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5 mb-6">
            <h2 className="font-semibold text-gray-700 mb-4">Approval Actions</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                {label:'Approve Leave',color:'bg-yellow-600'},
                {label:'Approve Promotion',color:'bg-purple-600'},
                {label:'Approve Transfer',color:'bg-orange-600'},
                {label:'Generate Invite Code',color:'bg-teal-600'},
              ].map((a) => (
